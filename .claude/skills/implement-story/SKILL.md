...
3. **Read UI design system** when the story touches screens or components: `docs/design/ui-reference.md` and primitives from `src/App.jsx`. Reuse existing primitives; do not introduce new color systems unless the story explicitly requires it.
...
4. **Produce an implementation plan** and print it to chat. Wait for user approval before phase 3.
   ...
   - **UI approach** (if applicable) — which primitives from `App.jsx` and which existing screen to mirror
