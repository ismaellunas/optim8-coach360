...
export class SupabaseProfileRepository implements ProfileRepository {
  ...
  async completePlayerOnboarding(id: string): Promise<Profile> {
    const { data, error } = await this.client.rpc('complete_player_onboarding');
    ...
    const row = data as Record<string, unknown>;
    if (!row || row.id !== id) {
      throw new Error('player_onboarding_completion_failed');
    }
  ...
  async logPlayerFirstDrill(id: string): Promise<Profile> {
    const { data, error } = await this.client.rpc('log_player_first_drill');
    ...
    const row = data as Record<string, unknown>;
    if (!row || row.id !== id) {
      throw new Error('player_first_drill_log_failed');
    }
